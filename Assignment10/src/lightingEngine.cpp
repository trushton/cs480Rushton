...
bool LightingEngine::initialize(string vs, string fs)
{
    ...
    //Now we link the 2 shader objects into a shaderProgram
    //This shaderProgram is what is run on the GPU
    ...
    directLight.Color = glm::vec3(1.0f, 1.0f, 1.0f);
    directLight.Direction = glm::vec3(0.0f, 0.0, 1.0);
...
void LightingEngine::toggleAmbientLight()
    {
        if(ambientLightOn)
            directLight.AmbientIntensity = 0.2f;
        else
...
void LightingEngine::toggleDiffuseLight()
    {
        if(diffuseLightOn)
            directLight.DiffuseIntensity = 0.7f;
        else
...
void LightingEngine::togglePointLight()
    {
        if(pointLightOn)
        {
            ...
            pointLights[0].Position = glm::vec3(0.0f, 8.0f, -16.0);
...
void LightingEngine::toggleSpotLight()
    {
        if(spotLightOn)
        {
            spotLights[0].DiffuseIntensity = 0.2f;
            spotLights[0].AmbientIntensity = 0.0f;
            spotLights[0].Color = glm::vec3(1.0f, 1.0f, 1.0f);
            spotLights[0].Position = glm::vec3(-32.0f, 8.0f, -32.0f);
            spotLights[0].Direction = glm::vec3(0.0f, 0.0f, 16.0f);
            spotLights[0].Attenuation.Linear = 0.9f;
